...
void user_read_data(png_structp png_ptr, png_bytep data, size_t length) {
    printf("user_read_data\n");
}

void user_write_data(png_structp png_ptr, png_bytep data, size_t length) {
    printf("user_write_data %d\n", length);
    int offset = 0;
    while (length > 0) {
        int bw;
        FRESULT result = f_write(png_get_io_ptr(png_ptr), data+offset, length, &bw);
        if (result != FR_OK) {
...
void user_flush_data(png_structp png_ptr) { printf("user_flush_data\n"); }
...
int writeImage(char* filename, int width, int height, float* buffer,
    ...
    FIL file;
    if (FR_OK != f_open(&file, filename, FA_CREATE_NEW | FA_WRITE)) {
        fp = NULL;
    } else {
        fp = (FILE*)&file;
    }
    ...
    png_set_compression_mem_level(png_ptr, 4);
